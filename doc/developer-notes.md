...
KZCash Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8277.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8276 for connections and services them.
...
However, in open source software such as KZCash Core, where everyone uses
...
pay attention to for reviewers of KZCash Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: KZCash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
