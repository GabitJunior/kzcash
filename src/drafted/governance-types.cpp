...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "UCOMNetwork") type = UCOMNetwork;
    if(strType == "UCOMNetworkVariable") type = UCOMNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == UCOMNetwork) s = "UCOMNetwork";
    if(type == UCOMNetworkVariable) s = "UCOMNetworkVariable";
